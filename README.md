...
开篇需要回答“读者看到了什么、为什么会这样、系统主要做了什么、怎样防止失控”。标题、摘要、核心结论和第一节不得直接堆叠未解释的缩写、英文实现标签、函数名或类型名。技术内容按“人话概念 → 项目术语 → 精确源码符号”递进；Wiki 卡片只用于快速复习，不能替代首次解释。页面标题和章节标题直接描述主题、结论或机制，避免把阅读步骤写成对读者的指令。

脚手架中的示例文字是占位说明，不应原样出现在成稿中。标题、导航和正文应直接陈述主题、现象、原因和处理机制，不要把阅读步骤写成口令。
