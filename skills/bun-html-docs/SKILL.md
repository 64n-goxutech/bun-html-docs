...
- Load the shared `assets/document-home.js` component exactly once. Do not hand-author additional home links in the top bar, sidebar, or page body. The component owns its appearance and resolves the documentation root for direct `file://` access and static hosting such as GitHub Pages.
...
- From the deepest relevant output path, confirm exactly one visible `docs-home-navigation` component exists and clicking it reaches the documentation root `index.html`.
